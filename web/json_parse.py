...
def jsonParse(jsonPath: str, weights: list) -> replyT:
    data = dict()
    score: float = 0.0
    allScores: list[float] = [0.0] * 9

    file = open(jsonPath, mode="r", encoding="utf-8")
    data = json.load(file)
    file.close()
    ...
    # 二. 计算得分
    # 1. 跳出率较高
    stayTime = float(round(stayTime, 2))
    if stayTime >= 30 * 1000:
        allScores[0] = 100
    ...
    allScores[0] = float(round(allScores[0], 2))  # 得分保留两位小数
    score += allScores[0] * weights[0]

    # 2. 重复点击
    allScores[1] = 60 + repeatClick[1] / \
        (repeatClick[0] + repeatClick[1]) * (100 - 60)

    allScores[1] = float(round(allScores[1], 2))
    score += allScores[1] * weights[1]

    # # 3. 页面打开慢
    # TODO 如果 json 里面没有 pageLoad 字段的处理（测试样例和其他的绝大多数情况不会）
    pageLoad = float(round(pageLoad / pageLoadCnt, 2))
    ...
    allScores[2] = float(round(allScores[2], 2))
    score += allScores[2] * weights[2]

    # 4. 点击后网络反馈慢
    feedbackInterval = float(round(feedbackInterval / feedbackIntervalCnt, 2))
    ...
    allScores[3] = float(round(allScores[3], 2))
    score += allScores[3] * weights[3]

    # 5. 点击无反应
    allScores[4] = 0.5 * allScores[2] + 0.5 * allScores[3]

    allScores[4] = float(round(allScores[4], 2))
    score += allScores[4] * weights[4]
    ...
    allScores[5] = float(round(allScores[5], 2))
    score += allScores[5] * weights[5]
    ...
    allScores[6] = float(round(allScores[6], 2))
    score += allScores[6] * weights[6]

    # 8. 页面加载白屏
    allScores[7] = 60 + isBlank[1] / \
        (isBlank[0] + isBlank[1]) * (100 - 60)

    allScores[7] = float(round(allScores[7], 2))
    score += allScores[7] * weights[7]
    ...
    allScores[8] = float(round(allScores[8], 2))
    score += allScores[8] * weights[8]

    score = float(round(score, 2))  # 总分同样保留两位小数
    ...
    briefDesc += f"5. 关于点击无反应，该项受页面打开和点击网络反馈的综合影响，得分 {allScores[4]} 分；\n"
    ...
    briefDesc += f"9. 关于多个同时出现，上述问题总共出现 {(100 - int(allScores[8])) // 5} 个，得分 {allScores[8]} 分。\n"
    ...
    detailDesc = gpt_35_api_stream(model, messages)
